...
- `lavish-review-workflow` - load before briefing planning or visual-review scouts, after scout done before presenting a review pack to the captain, and when reconciling scout completion gates that include a Lavish surface.
...
- `wire-design-workflow` - load before scaffolding or briefing a `product-wire-*` scout or `product-wire-preview-*` ship, and at FE ship intake when wire attestation is required.
